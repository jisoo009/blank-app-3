# streamlit_app.py
"""
Streamlit 앱: 해수온 대시보드 (한국어 UI) — OISST 기반
- NOAA OISST v2.1 사용 (Pathfinder 대체)
- 기상청 폭염일수(서울)
- 사용자 입력 예시 데이터
...
def load_public_datasets():
    ...
    try:
        ...
        dates = pd.date_range("2023-01-01", TODAY, freq="MS")  # 월별 예시
        dfs = []
        for d in dates:
            ym = d.strftime("%Y%m")
            date_str = d.strftime("%Y%m%d")
            file_url = f"{base_url}{ym}/oisst-avhrr-v02r01.{date_str}.nc"
            try:
                nc_bytes = download_text(file_url)
                tmp = f"/tmp/oisst_{date_str}.nc"
                with open(tmp, "wb") as f:
                    f.write(nc_bytes)
                ds = xr.open_dataset(tmp)
                da = ds["sst"]
                sst_mean = da.mean(dim=["lat","lon"]).to_series().resample("1M").mean()
                df_temp = sst_mean.reset_index()
                df_temp.columns = ["date","sst_global_mean_C"]
                dfs.append(df_temp)
            except Exception as e_inner:
                notices.append(f"OISST 파일 로드 오류: {file_url} — {e_inner}")
                continue
        if not dfs:
            raise ValueError("OISST 데이터 다운로드 실패")
...
def load_user_input_example():
    survey = pd.DataFrame({"response":["중요하게 인식함","보통","중요하지 않음"],"count":[128,45,27]})
    impacts = pd.DataFrame({"impact":["집중력 저하","수업 단축/취소","건강 문제(두통/탈수)","기타"],"percent":[45,25,20,10]})
...
dataset_choice = st.sidebar.radio("데이터셋 선택", ("NOAA 해수온 (OISST)","기상청 폭염일수 (서울)","사용자 입력 예시 데이터"))
...
period = st.sidebar.date_input("분석 기간 선택", [data_min,data_max], min_value=data_min,max_value=data_max)
analysis_option = st.sidebar.selectbox("분석 옵션 선택", ("추세 분석","계절성 분석","간단 요약 통계"))
